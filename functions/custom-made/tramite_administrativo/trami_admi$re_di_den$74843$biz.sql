create or replace function trami_admi$re_di_den$74843$biz(x$super number, x$tramite number, x$observaciones nvarchar2) return number is
    v$err           constant number := -20000; -- an integer in the range -20000..-20999
    v$msg           nvarchar2(2000); -- a character string of at most 2048 bytes?
    err_msg         nvarchar2(2000);
    v$estado        number;
    v$tipo          number;
begin --TramiteAdministrativo.revertirDictamenDenegar - business logic
    begin
      Select estado, tipo 
    ...
    update tramite_administrativo set observaciones = x$observaciones, estado = 1, fecha_transicion = current_date, usuario_transicion = util.current_user_id() where id = x$tramite;
