create or replace function lote$denegar$biz(x$super number, x$lote number, x$resolucion nvarchar2, x$fecha date, x$resumen nvarchar2, x$observaciones nvarchar2)
  return number is
...
begin
  For reg in (Select lp.pension, pn.estado, pe.codigo, ep.codigo as strestado
    ...
    if reg.estado=4 then
      contadord:=contadord+1;
      v$estado_final   := 5;
      update pension set resolucion_denegar = x$resolucion, fecha_resolucion_denegar = x$fecha, resumen_resolucion_denegar = x$resumen, 
  ...
  Update lote set observaciones='Resultado Resolución Denegar, registros denegados:' || contadord, cantidad=contador Where id=x$lote;
  return 0;
exception
  when others then
    err_msg := SQLERRM;
