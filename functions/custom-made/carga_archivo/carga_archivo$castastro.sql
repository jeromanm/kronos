...
begin
	...
	For reg in (Select * From csv_imp_temp Where archivo=x$archivo order by 1) loop
		...
		if contador>=contadoraux then
          ...
          v_cedula:=trim(substr(valor_columna,1,20));
	          ...
	          err_msg := SUBSTR(SQLERRM, 1, 200);
	  				x$reg:=carga_archivo$pistaerror(v_id_linea_archivo, 'Error al intentar obtener el monto del catastro, cédula:' || v_cedula || ', nombres:' || v_nombre || ',valor leido[' || valor_columna || '], mensaje:' || err_msg);
					END;
				else
					null;
        end case;
      ...
      when others then
					...
					err_msg := SUBSTR(SQLERRM, 1, 200);
					v_cant_errores:=v_cant_errores+1;
					x$reg:=carga_archivo$pistaerror(v_id_linea_archivo, 'Error al intentar obtener un registro anterior de registro de catastro para la cédula:' || v_cedula || ', nombres:' || v_nombre || ', mensaje:' || err_msg);
			End;
      Begin
      ...
      when others then
        ...
        x$reg:=carga_archivo$pistaerror(v_id_linea_archivo, 'Error al intentar crear el registro de catastro, cedula:' || v_cedula || ', nombres:' || v_nombre || ', número de línea:' || contador || ', mensaje:' || err_msg);
      END;
