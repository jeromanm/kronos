create or replace procedure cumple_no_indigena(x$cedula IN varchar2, x$cantidad IN OUT integer, x$cumple_regla IN OUT varchar2, x$tipo IN OUT varchar2,
...
BEGIN
  Begin
    Select 'Proveedor:' || fa.nombre || ', archivo:' || aa.archivo_cliente || ', fecha carga:' || to_char(ca.fecha_hora,'dd/mm/yyyy hh:mi AM') || ', nro Sime:' || nvl(es.codigo,'N/E') || ', comunidad:' 
            || de.NOMBRE_ENTIDAD || utl_match.jaro_winkler_similarity(upper(trim(de.nombre)),upper(trim(x$nombre_identificacion))), 
    ...
    From persona pe inner join no_indigena de on pe.id = de.persona
  ...
  when others then
    ...
    x$cantidad:=0;
    v$msg := SQLERRM;
    raise_application_error(v$err,'Error al intentar obtener el registro de no indígena, mensaje:' || v$msg,true);
  end;
  /*Begin
    Select case when (pe.indigena='false') then 0 else 1 end
      into x$cantidad
