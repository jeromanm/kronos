create or replace procedure cumple_nacimiento(x$cedula IN varchar2, x$cantidad IN OUT integer, x$cumple_regla IN OUT varchar2, x$tipo IN OUT varchar2,
...
BEGIN
  Begin
    Select 'Proveedor:' || fa.nombre || ', archivo:' || aa.archivo_cliente || ', fecha carga:' || to_char(ca.fecha_hora,'dd/mm/yyyy hh:mi AM') || ', nro Sime:' || nvl(es.codigo,'N/E') || ', fecha nacimiento:' 
            || nvl(to_char(de.FECHA_NACIMIENTOS,'dd/mm/yyyy'),'N/E') || utl_match.jaro_winkler_similarity(upper(trim(de.nombre)),upper(trim(x$nombre_identificacion))), 
    ...
    From persona pe inner join nacimiento de on (pe.codigo = de.cedula or pe.codigo = de.cedula_padre or pe.codigo=de.cedula_madre)
    ...
    Where pe.codigo=x$cedula And de.fecha_nacimientos is null 
      And rownum=1    
  ...
  when others then
    x$tipo:=null;
    v$msg := SQLERRM;
    raise_application_error(v$err,'Error al intentar obtener el registro de nacimiento, mensaje:' || v$msg,true);
  end;
  Begin
    Select case when (pe.fecha_nacimientos is null) then 0 else 1 end
      into x$cantidad
