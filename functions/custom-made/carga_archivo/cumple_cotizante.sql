create or replace procedure cumple_cotizante(x$cedula IN varchar2, x$cantidad IN OUT integer, x$cumple_regla IN OUT varchar2, x$tipo IN OUT varchar,
...
BEGIN
  Begin
    Select 'Proveedor:' || fa.nombre || ', archivo:' || aa.archivo_cliente || ', fecha carga:' || to_char(ca.fecha_hora,'dd/mm/yyyy hh:mi AM') || ', nro Sime:' || nvl(es.codigo,'N/E') || ', empresa:' 
            || de.nombres_empresa || ', % nombre:' || utl_match.jaro_winkler_similarity(upper(trim(de.nombre)),upper(trim(x$nombre_identificacion))),
    ...
    From persona pe inner join cotizante de on pe.id = de.persona or pe.codigo=de.cedula
    ...
    Where pe.codigo=x$cedula And de.fecha_egreso_cotizante is null 
      And rownum=1
  ...
  when others then
    x$tipo:=null;
    v$msg := SQLERRM;
    raise_application_error(v$err,'Error al intentar obtener el registro de Cotizante, mensaje:' || v$msg,true);
  end;
  Begin
    Select case when (pe.FECHA_EGRESO_COTIZANTE is null) then 0 else 1 end
      into x$cantidad
