create or replace procedure cumple_senacsa(x$cedula IN varchar2, x$cantidad IN OUT integer, x$cumple_regla IN OUT varchar2, x$tipo IN OUT varchar2,
...
BEGIN
  Begin
    Select 'Proveedor:' || fa.nombre || ', archivo:' || aa.archivo_cliente || ', fecha carga:' || to_char(ca.fecha_hora,'dd/mm/yyyy hh:mi AM') || ', nro Sime:' || nvl(es.codigo,'N/A') || ', cantidad semovientes:' 
            || de.cantidad_senacsa || utl_match.jaro_winkler_similarity(upper(trim(de.nombre)),upper(trim(x$nombre_identificacion))), 
    ...
    From persona pe inner join senacsa de on pe.id = de.persona
    ...
    Where pe.codigo=x$cedula And de.cantidad_senacsa is not null 
      And rownum=1
  ...
  Begin
    Select case when (pe.cantidad_senacsa is null) then 0 else 1 end
      into x$cantidad
