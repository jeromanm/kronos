create or replace procedure cumple_subsidio(x$cedula IN varchar2, x$cantidad IN OUT integer, x$cumple_regla IN OUT varchar2, x$tipo IN OUT varchar2,
...
BEGIN
  Begin
    Select 'Proveedor:' || fa.nombre || ', archivo:' || aa.archivo_cliente || ', fecha carga:' || to_char(ca.fecha_hora,'dd/mm/yyyy hh:mi AM') || ', nro Sime:' || nvl(es.codigo,'N/E') || ', empresa:' 
            || de.nombre_empresa || utl_match.jaro_winkler_similarity(upper(trim(de.nombre)),upper(trim(x$nombre_identificacion))), 
    ...
    From persona pe inner join subsidio de on pe.id = de.persona
  ...
  when others then
    x$tipo:=null;
    v$msg := SQLERRM;
    raise_application_error(v$err,'Error al intentar obtener el registro de Senacsa, mensaje:' || v$msg,true);
  end;
  Begin
    Select case when (pe.NOMBRE_EMPRESA_SUB is null) then 0 else 1 end
      into x$cantidad
