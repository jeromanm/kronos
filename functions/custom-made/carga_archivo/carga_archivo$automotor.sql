...
begin
    ...
    if contador=contadoraux then --encabezado del archivo
      ...
      For i in 0 .. cant_registro LOOP
        ...
        When 8 Then
          ...
          when others then
            ...
            x$reg:=carga_archivo$pistaerror(v_id_linea_archivo,'Error al intentar obtener el monto del vehículo, cédula:' || v_cedula || ', nombres:' || v_nombre || ', valor leido[' || valor_columna || '], mensaje:' || err_msg);
          END;
        else
          null;
        end case;
      ...
      when others then
        ...
        v_cant_errores:=v_cant_errores+1;
        err_msg := SUBSTR(SQLERRM, 1, 300);
        x$reg:=carga_archivo$pistaerror(v_id_linea_archivo,'Error al intentar buscar la cédula de la persona:' || v_cedula || ', nombres:' || v_nombre || ', mensaje:' || err_msg);
      End;
      v_id_automotor:=null;
				Begin
          if v_fecha_ingreso is null then
              ...
              And rownum=1; --buscamos registros anteriores de registro automotor con la misma fecha, para no repetir objeciones
				...
				when others then
					...
					v_cant_errores:=v_cant_errores+1;
					x$reg:=carga_archivo$pistaerror(v_id_linea_archivo, 'Error al intentar obtener un registro anterior de registro automotor para la cédula:' || v_cedula || ', nombres:' || v_nombre || ', mensaje:' || err_msg);
				End;
        Begin
        ...
        When others then
          ...
          v_cant_errores:=v_cant_errores+1;
          err_msg := SUBSTR(SQLERRM, 1, 300);
          x$reg:=carga_archivo$pistaerror(v_id_linea_archivo, 'Error al intentar crear el registro de automotor, cedula:' || v_cedula || ', nombres:' || v_nombre || ', número de línea:' || contador || ', mensaje:' || err_msg);
        END;
