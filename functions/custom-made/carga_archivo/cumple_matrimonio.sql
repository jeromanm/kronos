create or replace procedure cumple_matrimonio(x$cedula IN varchar2, x$cantidad IN OUT integer, x$cumple_regla IN OUT varchar2, x$tipo IN OUT varchar2,
...
BEGIN
  Begin
    Select 'Proveedor:' || fa.nombre || ', archivo:' || aa.archivo_cliente || ', fecha carga:' || to_char(ca.fecha_hora,'dd/mm/yyyy hh:mi AM') || ', nro Sime:' || nvl(es.codigo,'N/E') || ', fecha matrimonio:' 
            || de.fecha_matrimonio || utl_match.jaro_winkler_similarity(upper(trim(de.nombre1)),upper(trim(x$nombre_identificacion))), 
            utl_match.jaro_winkler_similarity(upper(trim(de.nombre1)),upper(trim(x$nombre_identificacion))) as porc_nombres, de.nombre1
      into x$tipo, v$porc, v$nombre
    From persona pe inner join matrimonio de on (pe.codigo = de.cedula1 or pe.codigo = de.cedula2)
    ...
    Where pe.codigo=x$cedula And (de.informacion_invalida is null or de.informacion_invalida<>'true') 
      And rownum=1    
  ...
  when others then
    x$tipo:=null;
    v$msg := SQLERRM;
    raise_application_error(v$err,'Error al intentar obtener el registro de matrimonio, mensaje:' || v$msg,true);
  end;
  Begin
    Select case when (pe.fecha_matrimonio is null) then 0 else 1 end
      into x$cantidad
