create or replace procedure cumple_fallecido(x$cedula IN varchar2, x$cantidad IN OUT integer, x$cumple_regla IN OUT varchar2, x$tipo IN OUT varchar2, 
...
BEGIN
  Begin
    Select 'Proveedor:' || fa.nombre || ', archivo:' || aa.archivo_cliente || ', fecha carga:' || ca.fecha_hora || ', nro Sime:' || es.codigo || ', fecha defunción:' || de.fecha_defuncion || ', % nombre:' || utl_match.jaro_winkler_similarity(upper(trim(de.nombre)),upper(trim(x$nombre_identificacion))), 
    ...
    From persona pe inner join defuncion de on pe.id = de.persona
  ...
  when others then
    x$tipo:='';
    v$msg := SQLERRM;
    raise_application_error(v$err,'Error al intentar obtener el registro de defunción, mensaje:' || v$msg,true);
  end;
  Begin
    Select case when pe.fecha_defuncion is null then 0 else 1 end as cantidad into x$cantidad
