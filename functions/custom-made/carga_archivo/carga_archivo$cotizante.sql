...
begin
	...
	For reg in (Select * From csv_imp_temp Where archivo=x$archivo order by 1) loop
        ...
        For i in 0 .. cant_registro LOOP
            ...
            When 4 Then
              ...
              when others then
                ...
                x$reg:=carga_archivo$pistaerror(v_id_linea_archivo, 'Error al intentar obtener el monto del cotizante, cédula:' || v_cedula || ', nombres:' || v_nombre || ', valor leido[' || valor_columna || '], mensaje:' );
              END;
            When 5 Then
          ...
          if v_fecha_ingreso is null then
	          ...
	          Where cedula=v_cedula And fecha_ingreso_cotizante=v_fecha_ingreso
				...
				when others then
					...
					err_msg := SUBSTR(SQLERRM, 1, 200);
					v_cant_errores:=v_cant_errores+1;
					x$reg:=carga_archivo$pistaerror(v_id_linea_archivo, 'Error al intentar obtener un registro anterior de cotizante para la cédula:' || v_cedula || ', nombres:' || v_nombre || ', mensaje:' || err_msg);
      End;
      Begin
      ...
      when others then
        ...
        x$reg:=carga_archivo$pistaerror(v_id_linea_archivo, 'Error al intentar crear el registro de cotizante, cedula:' || v_cedula || ', nombres:' || v_nombre || ', número de línea:' || contador || ', mensaje:' || err_msg);
      END;
