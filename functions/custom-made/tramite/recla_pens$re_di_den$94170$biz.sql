create or replace function recla_pens$re_di_den$94170$biz(x$super number, x$reclamo number, x$observaciones nvarchar2) return number is
...
begin
    begin --9.-revertir dictamen para reconsiderar denegar:
      Select estado, tipo
    ...
    /*if (v$tipo=1 or v$tipo=2) And (v$estado=5 or v$estado=4) then
      if (v$estado=4) then 
        ...
        Where id=v$pension;
        v$inserta_transicion := transicion_pension$biz(v$pension, v$fecha_dictamen, NULL, v$estado, v$estado_final, null, null, x$observaciones, v$nro_dictamen, v$fecha_dictamen, null, null, null, null);
      end if;
    else
