create or replace function pla_per_pa$a_s_p_o_p$74686$biz(x$super number, x$orden_pago number, x$numero_solicitud number) return number is
...
begin
  ...
  begin
    Select codigo into v$numero_solicitud
    From numero_solicitud Where id=x$numero_solicitud;
  exception
  when no_data_found then
    raise_application_error(v$err, 'Error: no se econtraron datos de la solicitud (STR).', true);
  when others then
    err_msg := SUBSTR(SQLERRM, 1, 200);
    raise_application_error(v$err, 'Error al intentar obtener el valor de la solicitud (STR), mensaje:' || err_msg, true);
  end;
  begin
  ...
  v$existe_str:=null; /*Se verifica que no se haya generado una orden de pago con la STR ingresada por RED BANCARIA*/
  p_existe_en_pla(x$ano, v$nen_codigo, v$ent_codigo, v$numero_solicitud, v$existe_str);
  if v$existe_str is not null then
    raise_application_error(v$err, 'Error: el STR ' || v$numero_solicitud || ' está asociado a una orden de pago por red bancaria.',true);
  end if;
  v$existe_str:=null; /*Se verifica que no se haya generado una orden de pago con la STR ingresada por CUENTA ADMINISTRATIVA.*/
  p_existe_en_pco(x$ano, v$nen_codigo, v$ent_codigo, v$numero_solicitud,v$existe_str);
  if v$existe_str is not null then
    raise_application_error(v$err, 'Error: el STR ' || v$numero_solicitud || ' está asociado a una orden de pago por cuenta administrativa.',true);
  end if;
  v$existe_str:=null;
  p_existe_solicitud_excep_str(x$ano, v$nen_codigo, v$ent_codigo, v$numero_solicitud, v$existe_str);
  if v$existe_str is not null then
    raise_application_error(v$err, 'Error: el STR ' || v$numero_solicitud || ' está asociado a una orden de pago por cuenta administrativa.',true);
  end if;
  v$existe_str:=null;
  p_existe_sol_exc_no_funciona(x$ano, v$nen_codigo, v$ent_codigo, v$numero_solicitud, v$existe_str);
  if v$existe_str is not null then
    raise_application_error(v$err, 'Error: el STR ' || v$numero_solicitud || ' está asociado a una orden de pago por cuenta administrativa NO FUNCIONARIO.',true);
  end if;
  begin
    ...
    Where a.ani_aniopre=x$ano
      ...
      And f.sol_numero=v$numero_solicitud;
  exception
  WHEN NO_DATA_FOUND THEN
    v_mes_sol:= 0;
    v_mes_act:=0;
  ...
  if v_mes_sol <> v_mes_act or v_mes_sol=0 or v_mes_act=0 then
    raise_application_error(v$err, 'Error: el mes de la solicitud nro:' || x$numero_solicitud || '(' || v_mes_sol || ') no corresponde al mes de pago o no se consiguen registros.', true);
  end if;
  if  v$gas_impoblig<>v$fob_impoblig or v$sol_importe<>v$monto_orden_pago then
    raise_application_error(v$err, 'Error: los montos asociados a la solicitud:' || v$numero_solicitud || ', gasto:' || v$gas_impoblig  || ', obligación:' || v$fob_impoblig || ', solicitud:' || v$sol_importe || ', son diferentes entre ellos o con el monto del resumen pensión:' ||  v$monto_orden_pago,true);
  end if;
  begin
    Update orden_pago set NUMERO_SOLICITUD=v$numero_solicitud
