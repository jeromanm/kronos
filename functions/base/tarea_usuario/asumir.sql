create or replace function tarea_usuario$asumir$biz(x$super number, x$tarea number) return number is
...
begin
    ...
    if v$row.destinatario <> v$log.id_usuario then
        v$msg := util.gettext('la notificacion esta dirigida a otro usuario');
        raise_application_error(v$err, v$msg, true);
    end if;
